---
title: "Hypothesis Testing: An Overview and Comparison of Permutation and Traditional Tests."
...
**Hypothesis testing** is a method for evaluating an assumption about a population based on sample data. In the statistics courses I teach, we cover both parametric and non-parametric methods for hypothesis testing. I have observed that students often struggle to select the most appropriate test. At times, it seems as though the primary objective of statistical classes is to teach how to choose the correct test, which is not entirely accurate.
...
In contrast, traditional statistical tests require the calculation of test statistics based on predefined distributions, such as the t-distribution or Chi-square distribution, to derive the p-value. These methods involve specific assumptions about the data (e.g., normality, equal variances), which guide the selection of the appropriate test.
...
perform_test <- function(data, variable, group_var) {
  ...
  observed_diff_median <- abs(median(group1) - median(group2))
  ...
  # Return test results including observed difference in medians
  list(
    Test_Type = test_type,
    p.value = test_result$p.value,
    Result = ifelse(test_result$p.value < 0.05, "Reject Null", "Fail to Reject Null"),
...
Wilcoxon_Results <- data.frame(
  ...
  p.value = sapply(results, function(x) x$p.value),
  Result = sapply(results, function(x) x$Result),
  Observed_Diff_Median = sapply(results, function(x) x$Observed_Diff_Median)
)
